...
public class Main {
    ...
    public static void turns(int i){
            if (!endGame){
                System.out.print("\nYou have " + (turns - i) + " turns left to escape.");
                if (inFoyer)
                    foyer();
                else if (inLibrary)
                    library();
    ...
    private static void intro(){
        System.out.print("\nYou wake up sprawled on a cold, ornate marble floor in what appears to be a dimly lit foyer. There are no visible windows, and the small amount" +
                ...
                "\nthe wall. To the east is a console table with a small box on top. A large, old-fashioned candle sits on a pedestal in the centre of the room. The" +
                "\ncandle is unlit.");
    }
    private static void help(){
        System.out.print("\nWhile playing, follow one verb with one noun, i.e. 'look around,' 'go north,' or 'open book. Do not use more than 2 words per statement.'\n");
    }
    ...
    private static void lookBox() {
        System.out.print("The box is an intricately designed wooden masterpiece, closed, but no visible locks.");
    }
    private static void readNote(){
        System.out.print("\nThe note simply says 'May my light show you the way.'");
    }
    private static void openChest(){
        System.out.print("\nInside the small box on the table you find an elegant cylindrical metal matchbox.");
        openBox = true;
    }
    ...
    private static void goThroughDoor2(){
        if (libraryDoorOpen) {
            System.out.print("\nBeyond the door lies yet another room, this one with an expensive-looking herringbone floor and mint green walls adorned with crown mouldings and mirrors. A " +
                    "\npearly white grand piano lies in the centre of the room, along with a silver French horn and a ceremonial Chinese gong. A stand in front of these all holds a piece of " +
    ...
    private static void lookAround() {
        System.out.print("\nAn expensive-looking herringbone floor and mint green walls adorned with crown mouldings and mirrors provide an opulent glimplse into someone's life." +
                "\npearly white grand piano lies in the centre of the room, along with a silver French horn and a ceremonial Chinese gong. A stand in front of these all holds a piece of " +
                "\nsheet music. To the North is yet another door, this one much larger than the previous ones and made of reinforced steel.");
    }
    ...
    private static void playHorn() {
        System.out.print("\nThe French horn lets out a sharp sound, and you think you hear a light click coming from the North door.");
        trumpetPlayed = true;
    }
    ...
    private static void playGong() {
        System.out.print("\nThe gong lets out a deep, resonating banging sound.");
        if (pianoPlayed){
            drumsPlayed = true;
            pianoPlayed = false;
            System.out.print("\nA screeching sound indicates that the final door has been unlocked.");
            endGame = true;
            completed = true;
        }
    }
